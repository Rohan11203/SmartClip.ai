...
const ClipForm = () => {
  ...
  const { currentStep } = useVideoProcessing(isOpen);
  ...
  async function handleSubmit() {
    const durationSec = toSec(data.endTime) - toSec(data.startTime);
    ...
    if (!data.url) {
      setError("Video URL is must");
      return;
    }
    if (durationSec > MAX_DURATION_SECONDS) {
      const limitInMinutes = MAX_DURATION_SECONDS / 60;
      setError(
        `Clip duration cannot exceed ${limitInMinutes} ${
          limitInMinutes === 1 ? "minute" : "minutes"
        }.`
      );
      return; // Stop the submission
    }
      ...
      <div className="w-full max-w-lg mx-auto">
        <div className="space-y-6">
          ...
          <button
            ...
            className="w-full py-3 bg-orange-600 hover:bg-orange-700 cursor-pointer disabled:bg-orange-200 dark:disabled:bg-orange-300 disabled:text-black text-white font-semibold rounded-md shadow-md transition flex items-center justify-center"
          >
            {loading ? (
              <>
